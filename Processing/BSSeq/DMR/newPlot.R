...
D1 <- read.table('AllDEInAno.mat',sep="\t",header = F)
...
Mapping1 <- read.table('mapping1.txt',sep="\t",header = F)

X <- data.frame(x=(D2$V9+D2$V8+D2$V7),  y=(D2$V10+D2$V11+D2$V12), genenames=Mapping1$V2 )
X <- na.omit(X)
...
ggsave(filename=paste("Test1.pdf",sep=""),width = 20, height = 20) 
Xhat$Ext <- 0
...
ggsave(filename=paste("Test1_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Primed") + ylab("Naive") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("Test1_v3.pdf",sep=""),width = 20, height = 20)
...
ggsave(filename=paste("Test1_v4.pdf",sep=""),width = 40, height = 40, p1)
saveext <- "./"
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/PN_Run1_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/PN_Run1_upper.csv",sep=""))




X <- data.frame(x=(D2$V9+D2$V8+D2$V7),  y=(D2$V13+D2$V14+D2$V15), genenames=Mapping1$V2 )
X <- na.omit(X)
genenames <- X$genenames
lrfit <- train(y~poly(x,degree=3), data=X, method = "lm")
predictedValues<-predict(lrfit,newdata = data.frame(x=X$x) )
...
geom_hline(yintercept = vals[1])
geom_hline(yintercept = vals[2])
ggsave(filename=paste("Test2_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Primed") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("Test2_v3.pdf",sep=""),width = 20, height = 20)
...
ggsave(filename=paste("Test2_v4.pdf",sep=""),width = 40, height = 40)
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/PTb_Run2_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/PTb_Run2_upper.csv",sep=""))

X <- data.frame(x=(D2$V10+D2$V11+D2$V12), y=(D2$V13+D2$V14+D2$V15), genenames=Mapping1$V2 )
X <- na.omit(X)
genenames <- X$genenames
lrfit <- train(y~poly(x,degree=3), data=X, method = "lm")
predictedValues<-predict(lrfit,newdata = data.frame(x=X$x) )
...
predictedValues2<-predict(lrfit,newdata = data.frame(x=xnew) )
vals <- quantile(Xhat$y,c(0.01,0.99))
ggplot(X, aes(x = x, y = y)) + geom_point(size=2.5)  + geom_point(color='blue') + theme_bw() + xlab("Naive") + ylab("Tb") +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
...
Xhat$Ext[which(Xhat$y>vals[2])] <- 2
Xhat$Ext <- as.factor(Xhat$Ext)
ggplot(Xhat, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Naive") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0"))+
geom_hline(yintercept = 0)+
geom_hline(yintercept = vals[1])+
geom_hline(yintercept = vals[2])
ggsave(filename=paste("Test3_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Naive") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("Test3_v3.pdf",sep=""),width = 20, height = 20)

p1 <- ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Naive") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0"))
p1 <- p1 + geom_text(data=X[c(which(Xhat$y<vals[1]),which(Xhat$y>vals[2])),],aes(x,y,label=genenames))
#geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
#p1 <- p1 + LabelPoints(plot = p1, points=c(which(Xhat$y<vals[1]),which(Xhat$y>vals[2])), labels = c(genenames[which(Xhat$y<vals[1])],genenames[which(Xhat$y>vals[2])]), repel = TRUE, color = 'red')
ggsave(filename=paste("Test3_v4.pdf",sep=""),width = 40, height = 40,p1)
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/NTb_Run3_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/Tb_Run3_upper.csv",sep=""))
