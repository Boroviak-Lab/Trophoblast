...
#These files havee the following order:
#Colum 7-9 Primed, 10-12 Niave,13-15 Tb
#Generated via:
...
#Mapping1 <- read.table('mapping2.txt',sep="\t",header = F)
...
#First we compare 7-9 vs 10-12 niave vs primed
X <- data.frame(x=(D1$V9+D1$V8+D1$V7)/3, y=(D1$V10+D1$V11+D1$V12)/3, genenames=Mapping1$V2 )
...
ggsave(filename=paste("PN_Test4.pdf",sep=""),width = 20, height = 20) 
Xhat$Ext <- 0
...
ggsave(filename=paste("PN_Test4_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Primed") + ylab("Naive") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("PN_Test4_v3.pdf",sep=""),width = 20, height = 20)
...
ggsave(filename=paste("PN_Test4_v4.pdf",sep=""),width = 40, height = 40, p1)
saveext <- "./"
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/PN_Run4_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/PNRun4_upper.csv",sep=""))


#ow we ccompare 7-9 vs 13-15, primede vs Tb
X <- data.frame(x=(D1$V9+D1$V8+D1$V7)/3, y=(D1$V13+D1$V14+D1$V15)/3, genenames=Mapping1$V2 )
...
predictedValues2<-predict(lrfit,newdata = data.frame(x=xnew) )
vals <- quantile(Xhat$y,c(0.01,0.99))
ggplot(X, aes(x = x, y = y)) + geom_point(size=2.5)  + geom_point(color='blue') + theme_bw() + xlab("Primed") + ylab("Tb") +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("PTb_Test5.pdf",sep=""),width = 20, height = 20)
Xhat$Ext <- 0
Xhat$Ext[which(Xhat$y<vals[1])] <- 1
Xhat$Ext[which(Xhat$y>vals[2])] <- 2
Xhat$Ext <- as.factor(Xhat$Ext)
ggplot(Xhat, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Primed") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0"))+
geom_hline(yintercept = 0)+
geom_hline(yintercept = vals[1])
geom_hline(yintercept = vals[2])
ggsave(filename=paste("PTb_Test5_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Primed") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("PTb_Test5_v3.pdf",sep=""),width = 20, height = 20)
p1 <- ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Primed") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
p1 <- p1 + geom_text(data=X[c(which(Xhat$y<vals[1]),which(Xhat$y>vals[2])),],aes(x,y,label=genenames))
...
ggsave(filename=paste("PTb_Test5_v4.pdf",sep=""),width = 40, height = 40)
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/PTb_Run5_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/PTb_Run5_upper.csv",sep=""))

#Now 10-13 vs 13-15

X <- data.frame(x=(D1$V10+D1$V11+D1$V12)/3, y=(D1$V13+D1$V14+D1$V15)/3, genenames=Mapping1$V2 )
X <- na.omit(X)
genenames <- X$genenames
lrfit <- train(y~poly(x,degree=3), data=X, method = "lm")
predictedValues<-predict(lrfit,newdata = data.frame(x=X$x) )
...
predictedValues2<-predict(lrfit,newdata = data.frame(x=xnew) )
vals <- quantile(Xhat$y,c(0.01,0.99))
ggplot(X, aes(x = x, y = y)) + geom_point(size=2.5)  + geom_point(color='blue') + theme_bw() + xlab("Niave") + ylab("Tb") +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("NTb_Test6.pdf",sep=""),width = 20, height = 20)
Xhat$Ext <- 0
Xhat$Ext[which(Xhat$y<vals[1])] <- 1
Xhat$Ext[which(Xhat$y>vals[2])] <- 2
Xhat$Ext <- as.factor(Xhat$Ext)
ggplot(Xhat, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Niave") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0"))+
geom_hline(yintercept = 0)+
geom_hline(yintercept = vals[1])+
geom_hline(yintercept = vals[2])
ggsave(filename=paste("NTb_Test6_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Niave") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("NTb_Test6_v3.pdf",sep=""),width = 20, height = 20)
...
#geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
#p1 <- p1 + LabelPoints(plot = p1, points=c(which(Xhat$y<vals[1]),which(Xhat$y>vals[2])), labels = c(genenames[which(Xhat$y<vals[1])],genenames[which(Xhat$y>vals[2])]), repel = TRUE, color = 'red')
ggsave(filename=paste("NTb_Test6_v4.pdf",sep=""),width = 40, height = 40,p1)
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/NTb_Run6_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/N_Tb_Run6_upper.csv",sep=""))
