...
X <- data.frame(x=log(D2$V16+1),  y=log(D2$V17+1), genenames=Mapping1$V2 )
X <- na.omit(X)
...
ggsave(filename=paste("Test7_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("EmDisc") + ylab("cMor") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("Test7_v3.pdf",sep=""),width = 20, height = 20)
...
ggsave(filename=paste("Test7_v4.pdf",sep=""),width = 40, height = 40, p1)
saveext <- "./"
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/Run7_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/Run7_upper.csv",sep=""))

X <- data.frame(x=log(D2$V16+1),  y=log(D2$V18+1), genenames=Mapping1$V2 )
X <- na.omit(X)
genenames <- X$genenames
lrfit <- train(y~poly(x,degree=3), data=X, method = "lm")
predictedValues<-predict(lrfit,newdata = data.frame(x=X$x) )
...
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("Test8.pdf",sep=""),width = 20, height = 20)
Xhat$Ext <- 0
Xhat$Ext[which(Xhat$y<vals[1])] <- 1
Xhat$Ext[which(Xhat$y>vals[2])] <- 2
Xhat$Ext <- as.factor(Xhat$Ext)
...
geom_hline(yintercept = vals[1])+
geom_hline(yintercept = vals[2])
ggsave(filename=paste("Test8_v2.pdf",sep=""),width = 20, height = 20)
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("EmDisc") + ylab("Tb") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("Test8_v3.pdf",sep=""),width = 20, height = 20)
...
ggsave(filename=paste("Test8_v4.pdf",sep=""),width = 40, height = 40)
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/Run8_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/Run8_upper.csv",sep=""))


X <- data.frame(x=log(D2$V17+1),  y=log(D2$V18+1), genenames=Mapping1$V2 )
X <- na.omit(X)
genenames <- X$genenames
lrfit <- train(y~poly(x,degree=3), data=X, method = "lm")
predictedValues<-predict(lrfit,newdata = data.frame(x=X$x) )
...
predictedValues2<-predict(lrfit,newdata = data.frame(x=xnew) )
vals <- quantile(Xhat$y,c(0.01,0.99))
ggplot(X, aes(x = x, y = y)) + geom_point(size=2.5)  + geom_point(color='blue') + theme_bw() + xlab("Tb") + ylab("cMor") +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
...
Xhat$Ext[which(Xhat$y>vals[2])] <- 2
Xhat$Ext <- as.factor(Xhat$Ext)
ggplot(Xhat, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Tb") + ylab("cMor") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0"))+
geom_hline(yintercept = 0)+
geom_hline(yintercept = vals[1])+
geom_hline(yintercept = vals[2])
...
X$Ext <- Xhat$Ext
ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Tb") + ylab("cMor") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
ggsave(filename=paste("Test9_v3.pdf",sep=""),width = 20, height = 20)
p1 <- ggplot(X, aes(x = x, y = y, color=Ext)) + geom_point(size=2.5)  + geom_point() + theme_bw() + xlab("Tb") + ylab("cMor") +
scale_color_manual(values=c("lightgrey", "#cc3761", "#457cb0")) +
geom_line(color='red',data = data.frame(x=xnew,y=predictedValues2), aes(x=x, y=y))
p1 <- p1 + geom_text(data=X[c(which(Xhat$y<vals[1]),which(Xhat$y>vals[2])),],aes(x,y,label=genenames)) 
#p1 <- p1 + LabelPoints(plot = p1, points=c(which(Xhat$y<vals[1]),which(Xhat$y>vals[2])), labels = c(genenames[which(Xhat$y<vals[1])],genenames[which(Xhat$y>vals[2])]), repel = TRUE, color = 'red')
ggsave(filename=paste("Test9_v4.pdf",sep=""),width = 40, height = 40)
...
write.csv(as.data.frame(genenames[which(Xhat$y<vals[1])]), file=paste(saveext,"/Run3_lower.csv",sep=""))
write.csv(as.data.frame(genenames[which(Xhat$y>vals[2])]), file=paste(saveext,"/Run3_upper.csv",sep=""))
